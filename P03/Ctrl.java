...
class Ctrl {
	...
	public void beginMainLoop(DataSource ds) {
		...
		while (true) {
			...
			case "schedulefl":
				...
				String seatInfo = textIO.prompt(prompt);
				String[] infoParsed = seatInfo.split("\\s+");
				
				consoleOut = scheduleFlight(depart, arrive, Short.parseShort(infoParsed[0]),
						Short.parseShort(infoParsed[1]), ds);
				textIO.display(consoleOut);
			    break;
	...
	private String scheduleFlight(String departureInfo, String arrivalInfo, short rows, short seatsPer, DataSource ds){
		...
		int status = ds.scheduleFlight(depCode, depDate, arrCode, arrDate, rows, seatsPer);
